...
void Domain::generate_grid(int n, int m){
    ...
    for(int i = 0; i <= n; ++i){
        for(int j = 0; j <= m; ++j){
            x_[i + j*(n+1)] = phi1(i*h1) * sides[3]->x(j*h2) + phi2(i*h1) * sides[1]->x(j*h2)
                             +phi1(j*h2) * sides[0]->x(i*h1) + phi2(j*h2) * sides[2]->x(i*h1)
                             -phi1(i*h1) * phi1(j*h2) * sides[0]->x(0)
                             -phi1(i*h1) * phi2(j*h2) * sides[3]->x(0)
                             -phi1(i*h1) * phi1(j*h2) * sides[1]->x(0)
                             -phi1(i*h1) * phi1(j*h2) * sides[2]->x(0);


            y_[i + j*(n+1)] = phi1(i*h1) * sides[3]->y(j*h2) + phi2(i*h1) * sides[1]->y(j*h2)
                             +phi1(j*h2) * sides[0]->y(i*h1) + phi2(j*h2) * sides[2]->y(i*h1)
                             -phi1(i*h1) * phi1(j*h2) * sides[0]->y(0)
                             -phi1(i*h1) * phi2(j*h2) * sides[3]->y(0)
                             -phi1(i*h1) * phi1(j*h2) * sides[1]->y(0)
                             -phi1(i*h1) * phi1(j*h2) * sides[2]->y(0);
